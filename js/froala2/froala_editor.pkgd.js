...
(function (factory) {
  ...
  $.FE.MODULES.html = function (editor) {
     ...
     */
    function normalizeSpaces (node, browser_way) {
      ...
      if (node.nodeType == Node.ELEMENT_NODE && ['STYLE', 'SCRIPT', 'HEAD'].indexOf(node.tagName) < 0) {
        ...
        for (var i = contents.length - 1; i >= 0 ; i--) {
          if (contents[i].tagName != Node.ELEMENT_NODE || (contents[i].className || '').indexOf('fr-marker') < 0) {
            normalizeSpaces(contents[i], browser_way);
          }
        }
      }
        ...
        if (_isBlock(prev_node) && _isBlock(next_node) && node.textContent.trim().length === 0 && editor.opts.enter != $.FE.ENTER_BR) {
          ...
          // Remove blank spaces if first.
          if (browser_way) {
            var p_node = node;
            while (editor.node.isFirstSibling(p_node) && !editor.node.isBlock(p_node)) {
              p_node = p_node.parentNode;
            }
            if (editor.node.isBlock(p_node) || (p_node && p_node.previousSibling && p_node.previousSibling.tagName == 'BR')) {
              txt = txt.replace(/^ */, '');
            }

            // New lines near blocks should collapse.
            if (editor.node.isBlock(prev_node)) txt = txt.replace(/^ */, '');
            if (editor.node.isBlock(next_node)) txt = txt.replace(/ *$/, '');
          }
          ...
          if (!node.nextSibling || editor.node.isBlock(node.nextSibling)) {
            new_text = new_text.replace(/ $/, $.FE.UNICODE_NBSP);
          }

          // Use space as a start if previous sibling is not block node or void.
  ...
  $.extend($.FE.DEFAULTS, {
    ...
    useClasses: true,
    spellcheck: true,
    iframeStyle: 'html{margin: 0px;}body{padding:10px;background:transparent;color:#000000;position:relative;z-index: 2;-webkit-user-select:auto;margin:0px;overflow:hidden;}body:after{content:"";display:block;clear:both}',
    direction: 'auto',
    zIndex: 1,
  ...
  $.FE.MODULES.core = function(editor) {
    function injectStyle(style) {
      if (editor.opts.iframe) {
        editor.$head.find('style[data-fr-style]').remove();
  ...
  $.FE.MODULES.commands = function (editor) {
    ...
    function _execCommand(cmd, tag) {
      ...
      if (editor.selection.isCollapsed() && editor.doc.queryCommandState(cmd) === false) {
        ...
        if (editor.selection.isCollapsed() && editor.doc.queryCommandState(cmd) === true && el.tagName == tag.toUpperCase() && (el.textContent || '').replace(/\u200B/g, '').length === 0) {
          $(el).replaceWith($.FE.INVISIBLE_SPACE + $.FE.MARKERS);
          editor.selection.restore();
        }
        else {
    ...
    function applyProperty (prop, val) {
      if (editor.selection.isCollapsed()) {
        ...
        if (editor.node.openTagString($parent.get(0)) == '<span style="' + prop + ': ' + $parent.css('font-size') + ';">' && editor.node.isEmpty($parent.get(0))) {
      ...
      if (!editor.node.isBlock(prev_node) && editor.node.isEditable(prev_node)) {
        ...
        if (prev_node.nodeType == Node.TEXT_NODE) {
          ...
          if (prev_node.textContent.length == 0) {
            if (prev_node.parentNode.childNodes.length == 2 && prev_node.parentNode == marker.parentNode && !editor.node.isBlock(prev_node.parentNode)) {
      ...
      if (!editor.opts.imagePaste) {
        ...
        editor.$el.find('img[data-fr-image-pasted]').each(function (index, img) {
          ...
          $(img)
            ...
            .addClass('fr-di' + (editor.opts.imageDefaultDisplay[0]) + (editor.opts.imageDefaultAlign != 'center' ? ' fr-fi' + editor.opts.imageDefaultAlign[0] : ''));
  ...
  $.FE.PLUGINS.link = function (editor) {
    ...
    function insert (href, text, attrs) {
      ...
      if (href.indexOf('tel:') !== 0 && href.indexOf('sms:') !== 0 && href.indexOf('mailto:') !== 0 && href.indexOf('notes:') !== 0 && href.indexOf('data:image') !== 0 && editor.opts.linkAutoPrefix !== '' && !/^(https?:|ftps?:|)\/\//.test(href)) {
        if (editor.opts.linkAutoPrefix[editor.opts.linkAutoPrefix.length - 1] == '/') {
          href = href.replace(/^\/*/g, '');
        }
